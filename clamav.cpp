...
public:
  ...
  std::pair<bool, std::string> scanSingleFile(const std::string &filepath) {
    ...
    if (!file) {
      logger.log(Logger::ERROR,
                 "Could not open file for scanning: " + filepath);
    ...
    if (!magicFile) {
      logger.log(Logger::ERROR,
                 "Could not open file for type detection: " + filepath);
      return {false, "error: file could not be opened for type detection"};
    }

    // Use a larger buffer for detecting signatures deeper in the file (like
    // ISO9660)
    ...
    try {
      ...
      if (bytes_read < 8) {
        logger.log(Logger::WARNING,
                   "File too small for reliable format detection: " + filepath);
      }
    } catch (const std::exception &e) {
      magicFile.close();
      logger.log(Logger::ERROR,
                 "Error reading file header: " + std::string(e.what()));
    ...
    // If we didn't detect by magic but the extension suggests an archive, log
    // it and try anyway
    if (!isArchive && (ext == ".tar" || ext == ".tgz" || ext == ".tbz" ||
                       ext == ".tbz2" || ext == ".zip" || ext == ".gz" ||
                       ext == ".bz2" || ext == ".rar" || ext == ".7z")) {
      logger.log(
          Logger::WARNING,
          "File has archive-like extension but signature not detected: " +
              filepath);
    ...
    if (!isArchive && ext == ".img") {
      logger.log(Logger::INFO,
                 "Found .img file, attempting detailed format detection: " +
                     filepath);
      ...
      // Check for ISO9660 signature (if we read enough bytes)
      else if (bytes_read >= 32774 && header[32769] == 'C' &&
               header[32770] == 'D' && header[32771] == '0' &&
               header[32772] == '0' && header[32773] == '1') {
        // It's an ISO image, which isn't a standard archive format ClamAV
        // handles well We'll use direct scanning but not extraction
        isArchive = false;
        ...
        logger.log(
            Logger::INFO,
            "Unknown binary format in .img file, will use default scanning");
      }
    }

    // Log the detected file type
    logger.log(Logger::INFO, "File type detected: " + detectedFileType +
                                 " for file: " + filepath);
    ...
    if (fileSize <= CHUNK_SIZE || isArchive) {
      ...
      int scan_result = cl_scanfile(filepath.c_str(), &virus_name, &scanned,
                                    engine, &scan_options);
      ...
      logger.log(
          Logger::DEBUG,
          "Total bytes scanned so far: " + std::to_string(total_bytes_scanned) +
              " bytes (" +
      ...
      if (scan_result == CL_VIRUS) {
        ...
        return {false, std::string("Error scanning file: ") +
                           cl_strerror((cl_error_t)scan_result)};
      }
    ...
    if (isArchive) {
      logger.log(Logger::INFO,
                 "Archive detected, extracting and scanning content: " +
                     filepath);
      ...
      if (tempDirPath == nullptr) {
        logger.log(
            Logger::ERROR,
            "Failed to create temporary directory for archive extraction");
        return {false,
                "error: could not create temporary extraction directory"};
      }
      ...
      if (isTarArchive) {
        ...
        if (basename.length() > 4 &&
            basename.substr(basename.length() - 4) == ".bz2") {
          basename = basename.substr(0, basename.length() - 4);
        }

        std::string outputFile = tempDir + "/" + basename;
        std::string extractCmd =
            "bzip2 -dc \"" + filepath + "\" > \"" + outputFile + "\"";
        logger.log(Logger::DEBUG, "Executing: " + extractCmd);
        int result = system(extractCmd.c_str());
        extractionSuccess = (result == 0);
      } else {
        // For other archive types, use libarchive directly or fall back to
        // ClamAV's built-in extraction
        logger.log(Logger::INFO, "Using ClamAV's built-in extraction for " +
                                     ext + " archive");
        ...
        if (scan_result == CL_VIRUS) {
          ...
          return {false, std::string("Error scanning archive: ") +
                             cl_strerror((cl_error_t)scan_result)};
        }
      ...
      if (!extractionSuccess) {
        ...
        logger.log(Logger::INFO,
                   "Falling back to ClamAV's built-in extraction");
        const char *virus_name = nullptr;
        unsigned long scanned = 0;
      ...
      try {
        ...
        logger.log(Logger::INFO, "Extracted " +
                                     std::to_string(extracted_files.size()) +
                                     " files from archive");

        // Scan each extracted file
        for (const auto &extracted_file : extracted_files) {
          logger.log(Logger::DEBUG,
                     "Scanning extracted file: " + extracted_file);

          const char *virus_name = nullptr;
          unsigned long scanned = 0;
          int scan_result = cl_scanfile(extracted_file.c_str(), &virus_name,
                                        &scanned, engine, &scan_options);
    ...
    logger.log(Logger::INFO,
               "Large non-archive file detected, using chunked scanning: " +
                   filepath);
    ...
    if (!largeFile) {
      logger.log(Logger::ERROR,
                 "Could not reopen file for chunk scanning: " + filepath);
    ...
    while (position < fileSize) {
      ...
      if (tempFilename.empty()) {
        logger.log(Logger::ERROR,
                   "Could not create temporary file for chunk scanning");
      ...
      const char *virus_name = nullptr;
      unsigned long scanned = 0;
      int scan_result = cl_scanfile(tempFilename.c_str(), &virus_name, &scanned,
                                    engine, &scan_options);
      ...
      logger.log(
          Logger::DEBUG,
          "Scanned chunk " + std::to_string(position / OVERLAP_SIZE) +
              " of file: " + filepath + " (" +
              std::to_string(position / (1024.0 * 1024.0)) + " MB - " +
              std::to_string((position + bytesToRead) / (1024.0 * 1024.0)) +
              " MB)");
    ...
    logger.log(Logger::INFO,
               "Completed chunked scan of file: " + filepath + " (" +
